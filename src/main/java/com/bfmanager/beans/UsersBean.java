...
import com.bfmanager.util.EmailUtils;
...
public class UsersBean implements Serializable {
	...
	public List<Users> getUsuariosGestores() {
		return usersService.getUsersManager();
	}
	...
	public List<Users> getUsuariosUsuario() {	
		return usersService.getUsersUser();
	}
	
	public List<Users> getUsuariosAvailable(Integer id_group) {
		List<Users> usuariosDisponibles = usersService.getUsersUser();
		List<UserGroup> usuariosAgregados = groupsService.getUserGroupxIdGroup(id_group);
		
		for (UserGroup userA : usuariosAgregados) {
			Users us = usersService.getUserXId(userA.getIdUser());		
			int iA = usuariosDisponibles.indexOf(us);			
			if (iA!=-1) usuariosDisponibles.remove(iA);
		}
		...
		try {
            ...
            while((auxt = br.readLine())!=null){            	
            	...
            	if(user_type!=-1) {
            		...
            		if(Integer.parseInt(linxlin[user_type])<1 || Integer.parseInt(linxlin[user_type])>3){
            			MsgUtil.msgError(LanguageBean.obtenerMensaje("error"), LanguageBean.obtenerMensaje("invalid_user_type")+", "+LanguageBean.obtenerMensaje("line")+" "+linea+".");
                		br.close();
                        f.delete();
                		return;
